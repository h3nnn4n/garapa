...
void test_create_buffer(_test_control *t) { t->test_bitmap = (uint32_t *)malloc(sizeof(uint32_t) * 144 * 160); }
...
void test_free_buffer(_test_control *t) { free(t->test_bitmap); }
...
uint32_t test_get_hash() {
    ...
    printf("#%3d [ %s " C_RESET "] 0x%08x -- %s\n", i + 1, hash == tests_to_run[i].hash ? C_GRN "PASS" : C_RED "FAIL",
           hash, tests_to_run[i].name);
...
void test_run() {
    ...
    for (int i = 0; i < 68; ++i) {
        ...
        printf("#%3d [ %s " C_RESET "] 0x%08x -- %s\n", i + 1,
               hash == tests_to_run[i].hash ? C_GRN "PASS" : C_RED "FAIL", hash, tests_to_run[i].name);
        test_reset_buffer(&test_control);
    }
