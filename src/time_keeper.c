...
void print_timer_state(_cpu_info *cpu) {
    /*printf(" CYCLES: %8llu /%8llu  TIMER: %4x  DIV: %2x  TIMA: %2x  TMA: %2x  TIMA_delay: %2d  Enable: %c  Speed:
     * %2x\n",*/
    printf("  Timer: DIV: %2x  TIMA: %2x  TMA: %2x  TIMA_reload_delay: %2d  TIMA_delay: %2d  Enable: %c  Speed: %2x\n",
           ...
           cpu->timer.TIMA_reload_timer, cpu->timer.TIMA_timer, cpu->timer.TAC & 0x04 ? '1' : '0',
           cpu->timer.TAC & 0x03);
}
...
void write_TAC(_cpu_info *cpu, uint8_t data) {
        ...
        switch (cpu->timer.TAC & 0x03) {
            case 0x00: // 00: 4096   Hz ( Increase every 1024 clocks )
                if (cpu->timer._timer & 0x0200) {
                    cpu->timer.TIMA++;
                    if (debug_timer)
                        printf("TIMA triggered on TAC write\n");
                }
                break;
            ...
            case 0x03: // 11: 16386  Hz ( 256 clocks )
                ...
                break;
            default: assert(0 && "Invalid value for TAC when writing to DIV\n"); break;
        }
    ...
    if (debug_timer)
        printf("TAC changed: %2x ->: Enable: %c  Speed: %2x\n", data, data & 0x04 ? 'y' : 'n', data & 0x03);
...
void write_TIMA(_cpu_info *cpu, uint16_t data) {
    if (debug_timer)
        printf("TIMA changed %2x to %2x, was: ", cpu->timer.TIMA, data);
    if (debug_timer)
        print_timer_state(cpu);
...
void timer_update(_cpu_info *cpu) {
    ...
    if (cpu->timer.TAC & 0x04) // If timer is running
        switch (cpu->timer.TAC & 0x03) {
            case 0x00: // 00: 4096   Hz ( Increase every 1024 clocks )
                if (!(cpu->timer._timer & 0x0200) && cpu->timer._timer_old & 0x0200) {
                    cpu->timer.TIMA++;
                }
                break;
            case 0x01: // 01: 262144 Hz ( 16 clocks  )
                if (!(cpu->timer._timer & 0x0008) && cpu->timer._timer_old & 0x0008) {
                    cpu->timer.TIMA++;
                }
                break;
            case 0x02: // 10: 65536  Hz ( 64 clocks  )
                if (!(cpu->timer._timer & 0x0020) && cpu->timer._timer_old & 0x0020) {
                    cpu->timer.TIMA++;
                }
                break;
            case 0x03: // 11: 16386  Hz ( 256 clocks )
                if (!(cpu->timer._timer & 0x0080) && cpu->timer._timer_old & 0x0080) {
                    cpu->timer.TIMA++;
                }
                break;
            default: assert(0 && "Invalid value for TAC\n"); break;
        }
