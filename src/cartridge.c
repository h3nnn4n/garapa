...
const char *mbc_code[] = {[0x00] = "ROM ONLY",
                          [0x01] = "MBC1",
                          [0x02] = "MBC1+RAM",
...
const char *rom_size_code[] = {
    [0x00] = " 32KByte (no ROM banking)", [0x01] = " 64KByte (4 banks)",   [0x02] = "128KByte (8 banks)",
    ...
    [0x52] = "1.1MByte (72 banks)",       [0x53] = "1.2MByte (80 banks)",  [0x54] = "1.5MByte (96 banks)"};
...
const char *ram_size_code[] = {[0x00] = "None",
                               [0x01] = "2 KBytes",
                               [0x02] = "8 Kbytes",
...
uint8_t get_ram_code(uint8_t *addr) { return *(addr + 0x0149); }

uint8_t get_rom_code(uint8_t *addr) { return *(addr + 0x0148); }

uint32_t get_ram_size(uint8_t *addr) { return ram_size[*(addr + 0x0149)]; }

uint32_t get_rom_size(uint8_t *addr) { return rom_size[*(addr + 0x0148)]; }
...
void print_rom_info(_cpu_info *cpu) {
    ...
    printf("ROM size: 0x%02x %s\n", cpu->mem_controller.memory[0x0148],
           rom_size_code[cpu->mem_controller.memory[0x0148]]);
    printf("RAM size: 0x%02x %s\n", cpu->mem_controller.memory[0x0149],
           ram_size_code[cpu->mem_controller.memory[0x0149]]);
...
uint16_t cartridge_read(_cpu_info *cpu, uint16_t addr) {
    ...
    if (addr >= 0xa000 && addr < 0xbfff) {
        ...
        /*printf("Reading from bank %x addr = %4x %4x \n", cpu->mem_controller.rom_bank_number, 0x4000 *
         * cpu->mem_controller.rom_bank_number + ( addr - 0x4000 ), addr );*/
        if (mapper) {
    ...
    switch (mapper) {
        ...
        case 0x01:
            if (addr < 0x2000) { // Ram Enable
                ...
                if (cpu->mem_controller.rom_bank_number == 0x00 || cpu->mem_controller.rom_bank_number == 0x20 ||
                    cpu->mem_controller.rom_bank_number == 0x40 || cpu->mem_controller.rom_bank_number == 0x60) {
            ...
            break;
        default: assert(0 && "Unsupported MBC "); break;
    }
